...
async def list_provider_models(
    ...
    try:
        if provider == "ollama":
            ...
            key_id = api_key_provider or "anthropic"
            api_key = await get_api_key(key_id)
            return await AnthropicProvider(base_url, api_key).models()
...
async def chat_stream(payload: ChatRequest) -> StreamingResponse:
    ...
    rate_limiter.check("ai_stream", max_requests=30, window_seconds=60.0)
...
async def get_context(payload: ContextRequest) -> dict:
    ...
    normalize_path(payload.workspace)
